...
+function($){
  ...
  DropList.prototype.drop = function() {
	   ...
	   this.$drop = $(this.options.isCanWrite?this.options.template1:this.options.template).appendTo(this.$element);
	   	if(this.options.hddName !=null) {
			if(!this.options.isCanWrite) {
				this.$element.append('<input type="text" style="position:absolute;left:-9999px;" name="'+this.options.hddName+'" id="'+this.options.hddName+'" />');			
			}else {
...
// 隐藏某项
DropList.prototype.hideItem = function(id) {
    this.$drop.find('ul').find('li[v="'+id+'"]').hide();
    var self = this;
    if(id == this.options.selectValue) {
        this.options.selectValue = '';
        this.setValue(this.options.selectValue);
...
// 显示某项
DropList.prototype.showItem = function(id) {
    this.$drop.find('ul').find('li[v="'+id+'"]').show();
}; 
  ...
  $.fn.hideItem = function(id) {
	  var droplist = this.data('bs.dropDown');
	  droplist.hideItem(id);	  
  }
  
  $.fn.showItem = function(id) {
	  var droplist = this.data('bs.dropDown');
	  droplist.showItem(id);	  
  }
