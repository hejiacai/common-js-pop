        ...
        _initPreviewActions: function () {
            ...
            $preview.find(btnRemove).each(function () {
                ...
                fnBefore = function (jqXHR) {
                    self.ajaxAborted = false;
                    self._resetErrors(false);
                    var ou = self._raise('filepredelete', [vKey, jqXHR, extraData]);
                    if (ou == false) return false;
                    if (self._abort()) {
                        jqXHR.abort();
                    } else {
                ...
                fnSuccess = function (data, textStatus, jqXHR) {
                    var ou = self._raise('filedeleted', [vKey, jqXHR, extraData]);
                    var n, cap;
                    if ((!$h.isEmpty(data) && !$h.isEmpty(data.error)) || !ou) {
                        params.jqXHR = jqXHR;
                        params.response = data;
                        self._showError(data.error || data.msg, params, 'filedeleteerror');
                ...
                self._handler($el, 'click', function () {
                    ...
                    self.ajaxAborted = false;
                    var ou = self._raise('filebeforedelete', [vKey, extraData]);
                    if (!ou) return false;
