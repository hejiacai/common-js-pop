 ...
 */
var cookieutility = {
	set:function(name, value,expiretime,path) { //设置cookie
		var expire = new Date();
		if(isNaN(expiretime) || expiretime=="") {
			expire.setFullYear(expire.getFullYear() + 20);
		}else {
	...
	del:function(name,path) {  //删除cookie
		var exp = new Date();
		...
		if(val!=null) document.cookie= name + "="+val+";expires="+exp.toGMTString()+path;		
	}	
}
