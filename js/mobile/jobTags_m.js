...
var myWindow={
	...
	//隐藏选择职业标签弹框
	$('.jobTagsDialog_m .getBack').on('click',function(){
	...
	$('#tagTrigger_m').on('click',function(){
		...
		$.post('/resumeV2/GetJobsortSkillTags ',
			{work_id:$('#work_id').val(),jobsort_id:$('#hidJobsort').val()||1413},
			function(res){
				var text = "";
				console.log(res)
	...
	$('.userTagDialogBtns a').on('click',function(){
		...
		switch (type){
			...
			$.post('/resumeV2/AddJobsortSkillTagCustom',
			{work_id:$('#work_id').val(),jobsort_id:$('#hidJobsort').val()||1314,tag_name:tag_name},
			function(res){
	...
	$('.dialogBody').on('click','.selectTags_m em i',function(){
		var $this=$(this);
		$.post('/resumeV2/DelJobsortSkillTagCustom',
