...
$(document).ajaxStart(function () {
    ...
    layer_index = layer.open({type: 2, shadeClose: false, content: layer_index_title});//换了种风格
});
 ...
 * @param is_empty 是否重置
 */
function location_href_set_get(get_obj, is_blank, is_empty) {
    ...
    is_blank ? open_new_bleak(urlGet('url') + '?' + GET_URL) : window.location.href = urlGet('url') + '?' + GET_URL;
}

function href_set_get(get_obj, is_empty) {
    var GET_URL = is_empty ? {} : urlGet();
    if (typeof get_obj != 'object' && typeof get_obj == 'string') {
        get_obj = split_get_param(get_obj);
    }
...
function get_form_data_array(id) {
    ...
    $(form_data).each(function (key, value) {
        ...
        //利用数组接收数据
        if (value.value && value.name.indexOf('[') != -1) {
            if (!re_data[value.name])
                re_data[value.name] = [];
            re_data[value.name].push(value.value);
        }else {
