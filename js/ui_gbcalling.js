 ...
 */ 
(function($) {
	var calling = function(element,opt) {
		this.id = '';
		this.dh = null; //
		this.dc = null; // 内容       
		this.df = null; // 底部
		...
		this._defaults ={
			 url: '//oa.boss.huibo.com/callingGB/getcalling/callback-callback-callingid-',
			 multipleUrl:'//oa.boss.huibo.com/callingGB/GetCallings/callback-callback-callingids-',
			 isLimit:false,
			 max: 3,
			 tipClass:'tipClass',
			 hddName:'hddName',
		...
		this.selectItem= function(data) {
			  var arr = new Array(),
				  ...
				  createItem = function(isSigle,pid,id,name,isdisabled,isChecked,isParent,isFirst,isShowControl) {
                      ...
                      var li_background;
                      if(isShowControl) {
                          li_background = 'style=" background: none;"';
                      }
                      s.Append('<li title='+name+' '+li_background+' d="'+id+','+name+'"');
					  if(isParent) {
			  ...
			  if(self.currLevel == 1) {
					// 一级职位类别
					$.each(data,function(i,n){
						 arr.push('<li title="'+n.calling_name+'" d="'+n.calling_id+','+n.calling_name+'"><label>'+n.calling_name+'</label></li>');
					});
					...
					$.each(data,function(i,n){
						 bool = true;
						 if (self.options.secondcalling.indexOf(n.calling_id)!=-1){
                             var item = createItem(isSingle,pid,n.calling_id,n.calling_name,self.isExists(pid),self.isExists(n.calling_id),false,false,false);
					...
					$.each(data,function(i,n){
                        ...
                        var item = createItem(isSingle,pid,n.calling_id,n.calling_name,false,self.isExists(n.calling_id),false,false,true);
                        arr.push(item);
					});
					if(bool) {
