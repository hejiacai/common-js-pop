    ...
    var weebox = function (opt) {
        ...
        this.initBox = function () {
            ...
            self.dh = $(html).appendTo('body').css({
                position: 'absolute',
                //    overflow: 'hidden',
                zIndex: getZIndex(opt.defind_index),
                left: -10000,
                top: -10000
            });
...
//          });
            if (!opt.noclick)
                self.dh.mousedown(function () {
                    self.dh.css('z-index', getZIndex(opt.defind_index))
                });
        ...
        this.setOncancel = function (fn) {
            ...
            if (typeof (fn) == "function") {
                var src = self.options.src;
                self.dh.find("._dialogClose,._dialogCancel").click(function () {
                    fn(src)
                });
            } else if (typeof (fn) == "number") {
                self.dh.find("._dialogClose,._dialogCancel").click(function () {
    ...
    /**
     * UPDATE:
     * options 新增参数 defind_index 自定义层级  noclick：true  禁用默认的点击增加层级功能
     *
     */
