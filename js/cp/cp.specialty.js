/**
 *  jQuery  ui_specialty.js  
 *  Copyright (c)  ZhangYu 
 */ 
(function($) {
	var category = function(element,opt) {
		this.id = '';
		this.dh = null; //
		this.dc = null; // 内容       
		this.df = null; // 底部 
		...
		this._defaults ={
			 url: 'http://cp.boss.huibo.com/schoolspecialty/GetSpecialty/callback-callback-degree_id-{degree}-specialty_id-',
			 multipleUrl:'http://cp.boss.huibo.com/schoolspecialty/GetSpecialtys/callback-callback-specialtys-',
			 isLimit:false,
			 max: 3,
			 tipClass:'tipClass',
			 hddName:'hddName',
			 inputName:'specialtyName',
			 selectItems:[],
			 selectClass:'cu',
		...
		this.initHtml= function(type) {
			 ...
			 var html =' <span><div class="dropSet"> '+
					   ...
					   ' <div class="dropLstHead"><a href="javascript:void(0)" class="unlimited btn1 btnsF12">类别不限</a><p>最多可选择<em></em>项</p><a class="closeDrop" href="javascript:void(0)">×</a></div>'+
						...
						' </div> '+
						' </div></span>';										
			 self.dh =$(html).appendTo(self.$element.empty()).find('.dropLst').hide();
			 ...
			 if(self.options.firstItem) {
				  url =url.replace('{degree}',self.options.firstItem);
			 }else {
			 	  url =url.replace('{degree}',0);
			 }
			 self.loadData(url,self.selectItem);
			 if(self.options.selectItems.length>0) {
			   	  var specialtyurl = self.options.multipleUrl+self.options.selectItems.join(',');	
				  self.options.selectItems = []; 	 
				  self.loadData(specialtyurl,self.setControl);
			 }
		...
		this.delItems = function(data) {
			$.each(data,function(i,n){
				if(self.options.selectItems.contains(n.specialty_id)) {
					self.del(n.specialty_id,n.specialty_name);
				}				
			});
		};
		...
		this.checkRadio = function() {
			var id = self.lastSelectItem[0],
			 ...
			 self.loadData(url,function(data){
				$.each(data,function(i,n){
					self.dh.find('li[d="'+n.specialty_id+','+n.specialty_name+'"]').find('input').attr('disabled','disabled');
				});					  
			 });*/
			 if(self.isExists(id)) {
				 return;
			 ...
			 self.hd.find('.seled[d]').each(function(){
				  ...
				  self.loadData(delurl,function(data){
						$.each(data,function(i,n){
							  self.dh.find('li[d="'+n.specialty_id+','+n.specialty_name+'"]').find('input').removeAttr('checked').removeAttr('disabled');				
						});					  
			 	 });*/ 
			...
			if(isChecked) {
				 ...
				 if(isSingle) { 
					 ...
					 self.loadData(url,function(data){
						self.delItems(data);
						$.each(data,function(i,n){
							 self.dh.find('li[d="'+n.specialty_id+','+n.specialty_name+'"]').find('input').attr('checked','checked').attr('disabled','disabled');		     
						});					  
					 });	
				   // 新增当前项到inPut中 
				   self.add(id,name);	
		...
		this.selectItem= function(data) {
			  ...
			  if(self.currLevel == 1) {
					// 一级职位类别
					$.each(data,function(i,n){
						 arr.push('<li d="'+n.specialty_id+','+n.specialty_name+'"><label>'+n.specialty_name+'</label></li>');
					});
					...
					$.each(data,function(i,n){
						 bool = true;
						 var item = createItem(isSingle,pid,n.specialty_id,n.specialty_name,self.isExists(pid),self.isExists(n.specialty_id),false,false,true);	
						 arr.push(item);
					});
					if(bool) {
			  ...
			  /*
					...
					$.each(data,function(i,n){
						 bool = true;
						 var item = createItem(isSingle,pid,n.specialty_id,n.specialty_name,self.isExists(pid),self.isExists(n.specialty_id));	
						 arr.push(item);
					});
					if(bool) {
		...
		this.setControl = function(data) {
			$.each(data,function(i,n){
				self.lastSelectItem[0] = n.specialty_id;
				self.lastSelectItem[1] = n.specialty_name;
				self.checkItem(true);
			});	   
		} 
		this.reset = function() {
		...
		this.setValue = function(items) {
			var values = items.split(','),
				...
				isSingle = self.isSingle();
			if(isSingle) {
				var url = self.options.multipleUrl+items;	
				self.loadData(url,self.setControl);
			}else {
				...
				if(newArr.length>0) {
					var url = self.options.multipleUrl+newArr.join(',');	 	 
					self.loadData(url,self.setControl);					
				}				
			}	 
  ...
  $.fn.category= function (option) {
	return this.each(function () {
	  ...
	  $this.data('bs.category', (data = new category(this, options)));
	  data.show();
  ...
  $.fn.setCategoryValue = function(ids) {
	  var category = $(this).data('bs.category');
	  category.setValue(ids);
  };
  
  $.fn.getCategoryValue = function() {
	  var category = $(this).data('bs.category');
	  return category.getValue();
 };
 $.fn.resetCategoryValue = function() {
	  var category = $(this).data('bs.category');
	  category.reset();
 };
