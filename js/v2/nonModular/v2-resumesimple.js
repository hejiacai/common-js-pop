...
hbjs.use('@editResume, @multipleSelect, @jobSorter, @fileUploader, @hbCommon, @jobDater, @areaSimple, @jobsort, @form, @calling, @areaMulitiple, @jobCertificate, @jobSkill, @fancyboxThumbs, @dateFormat', function(m){
	...
	baseInfoEditor.updatePreview = function(e){
		...
		this.getDom('#spnAvoirdupois').attr('v', txtAvoirdupois).text(txtAvoirdupois+'Kg');
	...
	workInfoEditor.resetData = function(){
		...
		if((preview = this._normal.eq(index)).length){
			...
			$('.jobTagsTxt div').html(tag2html(tempTagObj,true))
	...
	var eduInfoRules = {
		...
		trainInfoRules = {
			txtTrainInstitution: {required: true, range: [4,30]},
	...
	var eduTemplate = [
		...
		'<div class="box-item">',
		'<p class="tit">',
		'<span class="oper"><a href="javascript:" title="编辑" class="edt">编辑</a></span>',
		...
		'</p><p class="infor gray9 labelDetail" data-content="{labelDetail2}">{labelDetail}</p>',
		'</div></div>'
	].join('');
	eduInfoEditor.updatePreview = function(e){
		...
		if(!this._isAdd){
			...
			preview.find('.labelScore1').text(dataObj['labelScore1']);
			...
			preview.find('.labelDetail').html(dataObj['labelDetail'].replace(/\n/g, "<br/>"));
	...
	eduInfoEditor.on('delete', function(e){
		var typeHid = $('#eduTypeHid').val();
		var delNum = $('#'+typeHid+'Del').attr('data-num');

		var type = $('#dataTypeHid').val();
		var preview = this._normal.eq(e.index),
			dataType = type==1 ? 'data-trainid' : 'data-eduid',
			postType = type==1 ? 'train_id' : 'edu_id',
			operateType = type==1 ? 'DeleteTrain' : 'DeleteEdu',
			...
			self = this;
		data[postType] = this._normal.eq(delNum).attr(dataType);
	...
	projectInfoEditor.on('delete', function(e){
		var delNum = $('#projectDel').attr('data-num');
		var data = {
				'operate':'DeleteProject',
				'resume_id': resume_id,
				'project_id': this._normal.eq(delNum).attr('data-projectid')
			},
			self = this;
	...
	practiceInfoEditor.on('delete', function(e){
		var delNum = $('#practiceDel').attr('data-num');
		var data = {
				'operate':'DeleteWinning',
				'resume_id': resume_id,
				'practice_id': this._normal.eq(delNum).attr('data-practiceid')
			},
			self = this;
	...
	var otherTemplate = [
		...
		'<div class="infor">',
		'<span class="oper"><a href="javascript:" title="编辑" class="edt">编辑</a><a href="javascript:" class="del" title="删除">删除</a></span>',
		'<span class="topicContent" data-content="{topicContent2}">{topicContent}</span></div></div>',
	].join('');
	...
	otherInfoEditor.on('delete', function(e){
		var delNum = $('#otherDel').attr('data-num');
		var data = {
				'operate':'DeleteAppend',
				'resume_id': resume_id,
				'append_id': this._normal.eq(delNum).attr('data-appendid')
			},
			self = this;
