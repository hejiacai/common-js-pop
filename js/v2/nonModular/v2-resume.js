...
hbjs.use('@editResume, @multipleSelect, @jobSorter, @fileUploader, @hbCommon, @jobDater, @areaSimple, @jobsort, @form, @calling, @areaMulitiple, @jobCertificate, @jobSkill, @fancyboxThumbs, @dateFormat', function(m){
	...
	baseInfoEditor.resetData = function(){
		...
		if($('#spnIsPoor').length>0){
			attr=this.getDom('#spnIsPoor').attr("v");
			if(attr+1){
				dropPoor.setSelectedIndex(attr);
				var pp=this.getDom('#spnPoorPlace').attr('v')
				if(pp){
					$('#poorPlace').val(pp)
				}
			}else{
	...
	baseInfoEditor.updatePreview = function(e){
		...
		if($('#select_isPoor').length>0){
			var poorflag=$('#select_isPoor').val();
			if(poorflag){
				if(poorflag==1){
					$('#spnIsPoor').attr('v',poorflag).html('是')
					$('.poor-place-box').show()
					$('#spnPoorPlace').attr('v',$('#poorPlace').val()).html($('#poorPlace').val())
				}else{
					$('#spnIsPoor').attr('v',poorflag).html('否')
					$('.poor-place-box').hide()
				}
...
//			if($('#languageInfor .tag-box').children().length==0 && $('[name="hidLanguageTypeID[]"]').val()==""){
//				$('[name="hidLanguageTypeID[]"]').val()=="01"
//				$('#languageInfor .cancelBtn').click()
//				return false;
//			}
			console.log('提交了')
			var btn = e ? $(e.currentTarget) : this._submitButton,
...
//			console.log($('[name="hidLanguageTypeID[]"]'))
//			if($('#languageInfor .tag-box').children().length==0 && $('[name="hidLanguageTypeID[]"]').val()==""){
//				$('[name="hidLanguageTypeID[]"]').val()=="01"
//			}
