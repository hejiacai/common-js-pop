...
define('cqjob.jobDialog', function(require, exports, module){
    ...
    var weebox = function(opt)
    {
        ...
        this.initBox = function()
        {
            ...
            switch (self.options.type)
            {
                case 'anchor': //定位于屏幕某一位置的窗口
					...
					html = '<div class="_dialog" id="_dialog' + self.id + '">' +
						   ...
						   '    	<div class="prtCon">'+
						   '    		<i class="hbFntWes">' + iconFont +'</i><span class="_dialogContent prtTxt"></span><span class="clear"></span>' +
						   '    	</div>' +
						   '    </div>' +
						   '</div>';
                    break;
			...
			self.dh = $(html).appendTo('body').css({
				position: 'absolute',
				//    overflow: 'hidden',
				zIndex: self.options.zIndex || getZIndex(),
				left: -10000,
				top: -10000
			});
        ...
        this.initMask = function()
        {
            if (self.options.showMask)
            {
			    ...
			    else
			    {
				    ...
				    .appendTo('body').show().css(
				    {
				        zIndex: (self.options.zIndex - 1) || getZIndex()
				    });
					//.animate({opacity: 0.5},500)
			    }
            }
            ...
            } else if (self.options.contentType == "ajax")
            {
                ...
                $.ajax(
                {
                    ...
                    success: function(data)
                    {
                        ...
                        self.contentInited = true;
                        self.onopen();
						self.options.onSuccess && self.options.onSuccess.call(self.dh, data);
                    },
        ...
        this.initEvent = function()
        {
            ...
            self.dh.mousedown( function() { self.dh.css('z-index', self.options.zIndex || getZIndex()) });
        ...
        this.onclose = function()
        {
            ...
            {
                var src = self.options.src;
                return self.options.onclose.call(self.dh, src);
            }
        }
