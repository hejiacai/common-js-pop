...
define('cqjob.areaMulitiple', function(require, exports, module){
   ...
   var area = function(element,opt) {
		...
		this._defaults ={
			 url: '//www.huibo.com/area/getarea/callback-callback-areaid-',
			 multipleUrl:'//www.huibo.com/area/getarea/callback-callback-areaid-',
			 allSubUrl:'//www.huibo.com/area/getarea/callback-callback-all-1-areaid-',
			 singleUrl:'//www.huibo.com/area/getareas/callback-callback-areaids-',
			 isLimit:false,
			 max: 3,
			 tipClass:'tipClass',
			 hddName:'hddName',
		...
		this.initHtml= function(type) {
			 ...
			 var html =' <span><div class="dropSet"> '+
					   ...
					   '  </div>'+ 
					   ' <div class="dropLst">'+
					   ' <div class="dropLstHead"><a href="javascript:void(0)" class="unlimited btn1 btnsF12">类别不限</a><p style="font-size:14px">最多可选择<em></em>项</p></div>'+
						...
						'    </div>'+
						'    <div class="clear"></div>'+
						'<div class="btns"><a class="closeDrop" href="javascript:">确&nbsp;认</a></div>' + 
						' </div> '+
						' </div></span>';										
		...
		this.initEvent = function() {
			 ...
			 self.dh.find('.lst2 ul').click(function(e) {
				 ...
				 //单独判断如果是成都第三级不可选
				 var targetAreaId = $(target).closest('li').attr('d').split(',');
				 var thisHddName = self.options.hddName;
				 if(thisHddName == 'expArea'){
				 	if(targetAreaId[0] == 1501){
					 	var m = 1;
					 	var intervalx = setInterval(function(){
					 		if(++m > 20){
					 			clearInterval(intervalx);
					 		}
					 		if(self.dh.find('.lst3 ul li:first').length > 0){
					 			self.dh.find('.lst3 ul li').find('input').attr('disabled','disabled');
					 			self.dh.find('.lst3 ul li:first').find('input').removeAttr('disabled');
					 			clearInterval(intervalx);
					 	}
					 	},300);
				 }
			 ...
			 self.dh.find('.lst3 ul').click(function(e) {
				 ...
				 var targetID = $(target).closest('li').attr('d').split(',');
				  var thisHddName = self.options.hddName;
				 if(thisHddName == 'expArea'){
