...
(function (factory) {
	...
	if (typeof define === 'function'){
		define('cqjob.jobFormv2', factory);
	} else {
...
$.fn.submitForm = function(options)
{
    ...
    opt.success = function(fn, btn)
    {
        ...
        {
            if (opt.showRunning){
				opt.completeCallback ? opt.completeCallback.call(btn) : btn.stopRunning && btn.stopRunning();
			}
            try
            {
                if (fn)
    ...
    opt.complete = function(XMLHttpRequest, textStatus, errorThrown)
    {
        if (textStatus != 'success')
        {
            ...
            opt.errorCallback ? opt.errorCallback.call(btn, XMLHttpRequest, textStatus, errorThrown) : $.showError();
        }
        if (opt.showRunning){
			opt.completeCallback ? opt.completeCallback.call(btn) : btn.stopRunning && btn.stopRunning();
		}
        ...
        return function(data, frm)
        {
           ...
           // } catch (e) { throw new Error(e); }
            if (willSubmit && opt.showRunning){
				opt.startCallback ? opt.startCallback(btn) : btn.running && btn.running();
			};
            return willSubmit;
        };
...
//callback成功的回调
//error--失败的回调
$.fn.getJSON = function(url, data, callback, error, async, startCallback, completeCallback, errorCallback)
{
    ...
    success = function(fn, btn)
    {
        ...
        {
            completeCallback ? completeCallback.call(btn) : btn.stopRunning && btn.stopRunning();
            try
            {
                if (fn)
                {
    ...
    errorFn = function(XMLHttpRequest, textStatus, errorThrown)
    {
        ...
        {
            errorCallback ? errorCallback.call(btn) : $.showError && $.showError();
        }
    };
    startCallback ? startCallback.call(this) : this.running && this.running();
    $.ajax({
        type: "GET",
        url: url,
        dataType: "json",
...
//callback成功的回调
//error--失败的回调
$.fn.getHTML = function(url, data, callback, error, startCallback, completeCallback)
{
    var success = callback;
    var errorFn = null;
    var complete = null;
