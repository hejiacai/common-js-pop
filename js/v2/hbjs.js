...
(function(global,doc){
	...
	var hbjs = global.hbjs = {
			version: '1.384',
			autohor: 'lvke',
			mailto: 'boychina_20oo@qq,com'
		},
		...
		isObject = hbjs.isObject = isType("Object"),
		isString = hbjs.isString = isType("String"),
		isArray = hbjs.isArray = Array.isArray || isType("Array"),
		isFunction = hbjs.isFunction = isType("Function"),
		isBoolean = hbjs.isBoolean = isType("Boolean"),
		array = hbjs.arrayUtil = {
		...
		debug = hbjs.debug = function(msg, type){
		...
		var module = ('' + path ).split('::'),
			...
			if(!nodeCache[moduleType.loaded + modulePath]){
				...
				if(!nodeCache[moduleType.module + modulePath]){
					...
					var version = '';
					if(module[1]){
						normailzeVer(module[1]);
						version = comboModsVer[module[1]];
						version = '?v=' + (version ? version : parseInt(Math.random() * 100000000));
					}
					
					node.src   = modulePath + version;
					//console.log(modulePath + version);
					node.type  = "text/javascript";
					node.charset = charset;
					node.async = isAsync != undefined ? isAsync : true;
					node.defer = isAsync != undefined ? isAsync : true;
			...
			if(factory = cache[ moduleType.module + moduleName ].factory){
				...
				if(!result && !isBoolean(result)){
					comboExports[moduleName]
					 ...
					 = factory.apply(hbjs, args) || factory;
				}
	...
	function checkLoaded(moduleName, id){
		...
		if( status ){
			...
			for(var i=0; i<factorys.length; i++){
				if(isCombo(moduleName, i)){
					factorys[i] && factorys[i].call(hbjs, comboExports);
				} else {
					factorys[i] && factorys[i].apply(hbjs, save);
				}
		...
		require : function(module){
			...
			if(initId != undefined) {
				...
				if(combo){
					...
					if(comboPath){
						if(isDebug){
							debug('第一次加载JS:' + (comboHost || basepath) + comboPath);
						}
	...
	setBasePath();
	var curLoadMods = [];
	hbjs.use = function(ids, callback){
		if(isFunction(ids)){
			callback = ids;
			ids = null;
		}
	...
	hbjs.config = function(configData) {
	...
	hbjs.config({
		...
		map: {
			jquery: 'jquery.min',
			uploadify: 'cqjob/uploadify/uploadify'
		}
	});
