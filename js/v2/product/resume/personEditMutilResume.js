...
define('product.resume.personEditMutilResume',
	'product.resume.personEditResume',
function(require, exports, module) {

	var $ = module['jquery'],
		Class = require('base.class').Class,
		util = require('base.util'),
		editResume = module['product.resume.personEditResume'];

	var personEditMutilResume = Class(function(o){
        personEditMutilResume.parent().call(this, util.merge({
				element: $('#workInfor'),
				trigger: '.edt',
				addTrigger: '.edit',
				delTrigger: '.del',
    ...
    personEditMutilResume.implement({
		_initElements: function(){
            personEditMutilResume.parent('_initElements').call(this);
		...
		_initEvent: function(){
            personEditMutilResume.parent('_initEvent').call(this);
		...
		_cancel: function(e){
			...
			e.isCancel = true;
			this._addTrigger.show();
            personEditMutilResume.parent('_cancel').call(this, e);
		},
		_add: function(e){
			this.add();
			this.trigger('add', e);
		...
		add: function(){
            //添加统一处理
            var element = this.get('element');
            if(!isEditData && element.selector != '#'+scroll_flag){
                if(typeof isEditDataFun == 'function'){
                    return isEditDataFun();
                }
                return console.log('请指定处理方法{isEditData}');
            }
            //END 添加统一处理
			if(this._normalItem){
				this._edit.show();
		...
		_submit: function(e){
            personEditMutilResume.parent('_submit').call(this, e);
		},
		update: function(){
		...
		show: function(){
            ...
            personEditMutilResume.parent('show').call(this);
		}
	});

	return personEditMutilResume;
